        ...
        **********************************************************************
        ...
        if depth == 0:
            ...
            for m in game.get_legal_moves():
                ...
                v = self.min_value(child, depth-1)
                if v > best_score:
                    best_score = v
                    best_move = m
            return best_move
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def alphabeta_min_value(self, game, depth, alpha, beta):
        ...
        for m in moves:
            child = game.forecast_move(m)
            v = min(v, self.alphabeta_max_value(child, depth-1, alpha, beta))
            if v <= alpha: return v
            beta = min(beta, v)
        return v
    ...
    def alphabeta_max_value(self, game, depth, alpha, beta):
        ...
        for m in moves:
            child = game.forecast_move(m)
            v = max(v, self.alphabeta_min_value(child, depth-1, alpha, beta))
            if v >= beta: return v
            alpha = max(alpha, v)
        return v
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        if depth == 0 or not moves:
            ...
            for m in moves:
                child = game.forecast_move(m)
                v = self.alphabeta_min_value(child, depth-1, alpha, beta)
                if v > best_score:
                    best_score = v
                    best_move = m
                if best_score >= beta:
